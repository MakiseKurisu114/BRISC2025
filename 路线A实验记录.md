...
A1-A4 训练阶段只使用 `segmentation_task/train`，并从 train 内部划分 validation set 用于保存 best checkpoint。A5 使用固定 checkpoint 的 test 结果做综合比较，并选择后续调参的基础模型。
...
```text
A3：U-Net + Boundary Loss 是当前测试集整体最优模型。
```
