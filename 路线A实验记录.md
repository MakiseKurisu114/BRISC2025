...
A1-A4 训练阶段只使用 `segmentation_task/train`，并从 train 内部划分 validation set 用于保存 best checkpoint。`segmentation_task/test` 不参与训练，也不参与 checkpoint 保存；A5 使用固定 checkpoint 的 `eval_test` 结果做综合比较，并选择原始 A3 作为后续调参的基础模型。
...
- A3 是整体测试集 Dice / IoU 最优模型，因此后续 tuning 以 A3 为基础；
- glioma 和 small tumor 是分组分析中较困难的类别；
- small tumor 组的最优模型仍是 A1，说明 A3 tuning 需要重点关注小目标表现；
- 后续尝试 image_size、oversampling、boundary_weight、loss variant 和 threshold 等方向。
...
```text
A3 original：U-Net + Boundary Loss 是当前测试集整体最优模型。
```
...
- glioma 始终是最难分割的肿瘤类型；
- small tumor 仍是分组分析中的难点，后续若调参，应优先关注小目标；
- A5 支持后续以 A3 为基础做调参，例如 `boundary_weight`、image_size、oversampling、loss variant 和 threshold。
- 最终报告主模型采用原始 A3，test Dice = 0.8075，test IoU = 0.7271。
...
`image_size=192` 和 `image_size=256` 的 checkpoint 已保存，并作为 A3 tuning 候选配置纳入 validation-based selection。tuning 候选配置不根据本节的单项测试结果决定，而是在统一的 validation set 比较中选择。
...
根据 A5 的测试集分组分析，small tumor 是较困难的类别，因此尝试让训练阶段更频繁看到小目标样本。
...
small tumor oversampling 的 `w=1.5 / 2.0 / 3.0` checkpoint 均已保存，并作为 A3 tuning 候选配置纳入 validation-based selection。tuning oversampling 设置不根据本节的单项测试结果决定，而是在统一的 validation set 比较中选择。
...
tuning threshold 由 validation set 选择，当前为 `0.30`。
...
Focal Tversky w=0.2 checkpoint 已保存，并作为 A3 tuning 候选配置纳入 validation-based selection。tuning loss 变体不根据本节的单项测试结果决定，而是在统一的 validation set 比较中选择。
...
`boundary_weight = 0.05 / 0.1 / 0.3 / 0.5` 的 checkpoint 均已保存，并作为 A3 tuning 候选配置纳入 validation-based selection。tuning boundary weight 不根据本节的单项测试结果决定，而是在统一的 validation set 比较中选择。
...
validation-based tuning 选出的候选配置：
...
## A3 tuning：validation-based supplemental tuning
...
test split : 只对 validation-based tuning 固定候选配置做一次泛化评估
```
...
```text
最终报告主模型 = 原始 A3（U-Net + Boundary Loss）
原始 A3 test Dice = 0.8075
原始 A3 test IoU  = 0.7271
```

A3 tuning 表明进一步调整 boundary weight、输入分辨率、小目标过采样、loss variant 和 threshold 后，validation 指标可出现小幅变化；其中 validation-based tuning 选出的候选配置为 `A3_boundary_w03 + threshold 0.30`。但该固定候选配置的 final test Dice = 0.7986、final test IoU = 0.7164，未超过原始 A3。因此本文最终仍以原始 A3 作为主结果，A3 tuning 作为补充分析和后续优化探索。
...
```text
outputs/final_model/
outputs/final_model/README.md
outputs/final_model/final_model.json
```

该汇总只读取已有结果文件，不重新训练，不重新生成 csv/png/pt，也不复制 `.pt` 权重文件。final model 的选择依据写入 `outputs/final_model/final_model.json`。

选择流程：

```text
1. 从 outputs/a5/summary/overall_test_metrics.csv 选择 A1-A4 中 independent test Dice 最高的主实验模型；
2. 若 test Dice 接近或并列，再比较 test IoU；
3. 读取 outputs/a3_tuning/final_selection.json，确认 A3 tuning candidate 来自 validation-based selection；
4. 读取 outputs/a3_tuning/final_test.csv，将 tuning final candidate 与主实验最佳模型比较；
5. 只有 tuning final test Dice / IoU 超过主实验最佳模型时才替代 final model。
```

当前自动选择结果：

| final model | method | checkpoint | threshold | test Dice | test IoU |
|---|---|---|---:|---:|---:|
| A3 | U-Net + Boundary Loss | `outputs/a3/full/checkpoints/best_unet_boundary.pt` | 0.50 | 0.8075 | 0.7271 |

A3 tuning 的 validation-based candidate 为 `A3_boundary_w03 + threshold 0.30`，final test Dice = 0.7986，final test IoU = 0.7164，未超过原始 A3。因此 final model 仍采用原始 A3，A3 tuning 作为 supplemental tuning experiment 保留。

---

## Final inference and visualization：最终模型展示

状态：已完成。

本阶段读取 `outputs/final_model/final_model.json` 中已选定的 final model，不重新训练，不修改数据集，不复制或修改 `.pt` checkpoint。最终推理使用：

```text
model      = A3（U-Net + Boundary Loss）
...
运行后新增 / 更新到 `outputs/final_model/` 下：
...
final inference 整体结果：

| split | n | Dice | IoU | Precision | Recall |
|---|---:|---:|---:|---:|---:|
| segmentation_task/test | 860 | 0.8069 | 0.7263 | 0.8221 | 0.8442 |

已生成 overall / tumor / view / size 四类可视化：

```text
outputs/final_model/figures/overall_examples.png
outputs/final_model/figures/tumor_glioma_examples.png
outputs/final_model/figures/tumor_meningioma_examples.png
outputs/final_model/figures/tumor_pituitary_examples.png
outputs/final_model/figures/view_axial_examples.png
outputs/final_model/figures/view_coronal_examples.png
outputs/final_model/figures/view_sagittal_examples.png
outputs/final_model/figures/size_small_examples.png
outputs/final_model/figures/size_medium_examples.png
outputs/final_model/figures/size_large_examples.png
```
...
- best：组内 Dice 最高；
- worst：组内 Dice 最低；
- typical：Dice 最接近组内中位数；
- overall 图选择 2 个 best、2 个 typical 和 2 个 worst。

主要观察：

- meningioma 表现最好，glioma 仍是最困难的 tumor type；
- axial / coronal / sagittal 三个视角整体较稳定，sagittal 的平均 Dice 略高；
- small tumor 组 Dice = 0.7542，低于 medium / large，仍是主要困难点；
- 成功案例中 GT 和预测轮廓高度重合，失败案例常见于小病灶或低对比度区域，部分 worst 样本 Dice 接近 0。
