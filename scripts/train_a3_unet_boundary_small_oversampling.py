...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="A3 tuning: U-Net + Boundary Loss with small tumor oversampling")
    ...
    parser.add_argument("--small-sample-weight", type=float, default=3.0)
    parser.add_argument("--val-ratio", type=float, default=0.2)
    parser.add_argument("--limit-train", type=int, default=None)
    parser.add_argument("--num-workers", type=int, default=0)
    parser.add_argument("--out-dir", type=Path, default=Path("outputs/a3_tuning/small_oversampling_w3/full"))
...
def build_small_oversampling_weights(dataset, small_threshold: float, small_sample_weight: float) -> tuple[torch.Tensor, dict]:
    weights = []
    counts = {"small": 0, "non_small": 0}
    area_ratios = []
    for _, mask_path in dataset.pairs:
        area_ratio = mask_area_ratio(mask_path)
        area_ratios.append(area_ratio)
        if area_ratio < small_threshold:
            weights.append(small_sample_weight)
            counts["small"] += 1
        else:
            weights.append(1.0)
            counts["non_small"] += 1
    ...
    weighted_small_probability = (
        sum(weight for weight, area_ratio in zip(weights, area_ratios) if area_ratio < small_threshold) / total_weight
    )
    summary = {
        "small_threshold": small_threshold,
        "small_sample_weight": small_sample_weight,
        "small_count": counts["small"],
        "non_small_count": counts["non_small"],
        "small_fraction": counts["small"] / len(dataset),
        "weighted_small_probability": weighted_small_probability,
    }
...
def main() -> None:
    ...
    sample_weights, sampling_summary = build_small_oversampling_weights(
        train_set,
    ...
    print(f"small_threshold={args.small_threshold} small_sample_weight={args.small_sample_weight}")
    print(
        "sampling "
        f"small_count={sampling_summary['small_count']} "
        f"non_small_count={sampling_summary['non_small_count']} "
        f"small_fraction={sampling_summary['small_fraction']:.4f} "
        f"weighted_small_probability={sampling_summary['weighted_small_probability']:.4f}"
    )

    sampling_path = args.out_dir / "sampling_summary.csv"
    with sampling_path.open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            [
                "small_threshold",
        ...
        writer.writerow(
            [
                sampling_summary["small_threshold"],
                sampling_summary["small_sample_weight"],
                sampling_summary["small_count"],
                sampling_summary["non_small_count"],
                sampling_summary["small_fraction"],
                sampling_summary["weighted_small_probability"],
            ]
        )
    ...
    plot_training_history(
        history_path,
        args.out_dir / "figures" / "training_curves.png",
        "A3 U-Net + Boundary Loss + Small Tumor Oversampling",
    )
