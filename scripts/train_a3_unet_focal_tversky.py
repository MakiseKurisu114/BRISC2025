...
from src.losses import BCEDiceBoundaryFocalTverskyLoss
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="A3 tuning: U-Net + BCE + Dice + Boundary + Focal Tversky Loss"
    )
    ...
    parser.add_argument("--focal-tversky-weight", type=float, default=0.2)
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("outputs/a3_tuning/focal_tversky_w02/full"))
...
def main() -> None:
    ...
    criterion = BCEDiceBoundaryFocalTverskyLoss(
    ...
    print(
        "focal_tversky "
        f"weight={args.focal_tversky_weight} "
        f"alpha={args.focal_tversky_alpha} "
        f"beta={args.focal_tversky_beta} "
        f"gamma={args.focal_tversky_gamma}"
    )
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if val_dice > best_dice:
                ...
                torch.save(
                    ...
                    args.out_dir / "checkpoints" / "best_unet_boundary_focal_tversky.pt",
                )
    ...
    plot_training_history(
        ...
        "A3 U-Net + Boundary + Focal Tversky Loss",
    )
    print(f"saved: {history_path}")
    print(f"saved: {args.out_dir / 'checkpoints' / 'best_unet_boundary_focal_tversky.pt'}")
